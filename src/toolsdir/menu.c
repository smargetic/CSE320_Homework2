#include <stdio.h>
//#include <varargs.h> //No longer supports, so, I took it out
#include "ctools.h"
#include "menu.h"
#include "sys5.h"
...
/*
void setValVAList(char* setVal, char* compareVal, int len, char** listVA, int index){
  //I check if the set values are equal
  if(strncmp(setVal, compareVal, len)==0){
    listVA[index] = setVal
  } else {
    //there was nothing passed, so
    listVA[index] = "0"
  }
...
/*
char** va_list(char* no, char* yes, char* help, char* abort, char* data){
  char** listVA=malloc(5*sizeof(char*));
...
*/
/*
void setVAList(int* list, char* setVal, char* compareVal, int len, int index){
  //If the two are equal, and something is there, I set it to 0
  if(strncmp(setVal, compareVal, len)==0){
    list[index] = 0;
  } else {
    //there was nothing passed, so I set it to 0
    list[index] = 0;
  }
}
int* va_list(char* no, char* yes, char* help, char* abort, char* data){
  int* list = malloc(5*sizeof(int));
  setVAList(list, "No", no, 2, 0);
  setVAList(list, "Yes", yes, 3, 1);
  setVAList(list, "Help", help, 2, 2);
  setVAList(list, "Abort", abort, 2, 3);
  setVAList(list, "Data", data, 2, 4);
  return list;
}
...
int menu_match (int* ptr_rval, char** ptr_ur,char* prompt,int casetest,int isub,int r_no_match,int r_ambiguous,int n_options, ... ){
  ...
  //int totalNum = 2*n_options;
  //I CHANGED THIS --> NOT SURE IF RIGHT
  va_start(pvar, n_options); //I CHANGED THE SECOND VARIABLE TO N_OPTIONS
  ...
  printf("%s",prompt);
  switch (rval = recieveLine(stdin,line,MAX_MENU_RESPONSE_LENGTH)) {
    case AT_EOF :
      return(MENU_EOF);
      break;
    case TOO_MANY_CHARS :
...

{
  ...
  return(rval); //I ADDED THIS
}

//took out extern from this
int menu_data_help_or_abort (char *prompt,char *abortstring,char **ptr_response)

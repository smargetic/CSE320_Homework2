...
//void fill (addr,ch,n) register char *addr, register char ch, register int n;
//void copy (dest,src,n) register char *dest,*src, register int n;
void fill(char* addr, char ch, int n);
...
int nbuffconcat(char* buffer, int n, ...)
{
  ...
  for (i = 1; i <= 6; i++) {
      ...
      switch (i) {
        //case 1 : s = s1; break;
        case 1: s= va_arg(v1, char*);
                break;
        case 2 : s= va_arg(v1, char*);
                break;
        case 3 : s= va_arg(v1, char*);
                break;
        case 4 : s= va_arg(v1, char*);
                break;
        case 5 : s= va_arg(v1, char*);
                break;
        case 6 : s= va_arg(v1, char*);
                break;
      }
      while (*s != '\0') *b++ = *s++;
  }
...
int slge_compare (char* s1,int l1,char* s2,int l2) //NEED TO DOUBLE CHECK THIS
...
//  char *s2;
//  Bool fast;
char* strfind(char* s1, char* s2, Bool fast)
{
...
//  char *s2;
//  Bool fast;
char* strncfind(char* s1, char* s2, Bool fast)
{
  //register int len1,len2;
  int len1;
  int len2;
...
char* strsearch(char* s1, char* s2, int s1len, int s2len)
  {
    //register char *pc;
    // register char *bound;
...
//  register char *s1, *s2;
//  register int s1len,s2len;
char* strncsearch(char* s1, int s1len, char* s2, int s2len)
  {
...

{
  ...
  if (posttrim) {
     ...
     while (T) {
       ch = *np;
       trim = (0 != index(trimchars,ch));
       if (trim) *np = '\0'; //CHANGED THIS FROM *np == '\0' to = '\0'
...
char* string_upcase (char *astring)
{
  ...
  return(astring); //ADDED RETURN STATEMENT
}
...

{
  ...
  while (T) {
    ...
    switch (len = recieveLine(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int recieveLine (FILE* iport,char* buffer, int buflen)
...

{
  int len;
  char *line;
  if (0 > (len = recieveLine(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
        *ptr_lines = (char **) emalloc(n * sizeof(char **));
        //free(*ptr_lines); //I WROTE THIS <-- NOT SURE IF CORRECT
        return(n);
     }
     else {
        return(TOO_MANY_CHARS);
...

{
  ...
  while (T) {
    ...
    if (0 > (len = recieveLine(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }

...

{
  ...
  //,oldentry,currententry; --> UNUSED
  Ptr_Rolo_List newlink,rptr;
  char *next_field,*next_other;
  //,*oldname,*currentname; --> UNUSED
  char **other_pointers;
  int n_entries = 0;
...
{
  //I check if value is null
  if(value ==NULL){
    //I return if not
    return;
  }
  //int semi = 0; //COMMENTED OUT SEMI
  int i;
...
void cathelpfile (filepath,helptopic,clear) //changed from int to void
